...
// Wind Gust Layer
export function loadWindSpeedLayer(map, layers) {
    const windLayer = layers?.['windSpeed'];
    if (!windLayer) return;

    // initial paths
    let gustPath = buildDataPath('wspd.json');
    let imageUrl = buildDataPath('wspd.png');
    ...
    function iconForValue(value, item) {
        ...
        if (angleStr === '0' && speedStr === '0') {
            icon = L.icon({ iconUrl: filename, iconSize: [6, 6], iconAnchor: [14, 14], popupAnchor: [0, -14] });
        }
    ...
    async function update() {
        ...
        addDataMarkers(map, windLayer, normalized, {
            popupFormatter: it => `<strong>Wind Speed:</strong> ${it.value ?? 'n/a'} mph from ${it.angle ?? 'n/a'}°`,
    ...
    const unsubscribe = onTimestampChange(() => {
        gustPath = buildDataPath('wspd.json');
        imageUrl = buildDataPath('wspd.png');
